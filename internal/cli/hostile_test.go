...
func TestNonSandboxableWorktreeIsRefusedWithoutCreatingAnything(t *testing.T) {
	...
	f, _, err := runFullRoot(t, home, "up", "api", "-w", "+wip")
	if err == nil {
...
func TestANestWithNoRepoStillMountsTheAgentProfile(t *testing.T) {
	...
	f, _, err := runFullRoot(t, home, "up", "api")
	if err != nil {
...
func TestRelativeWorktreeRootIsRefusedNamingTheField(t *testing.T) {
	...
	f, _, err := runFullRoot(t, home, "up", "api", "-w", "feat1")
	if err == nil {
...
//
// Measured on the binary before the fix: `den up api` failed printing the YAML
