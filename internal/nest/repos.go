...
//
// A key-typed entry is named "key <k>", not by its path: it HAS none before
// Resolve, and a bare empty string would leave the reader with half a
// collision to hunt. The AdHoc arm comes first without ambiguity — a
// positional's Path is never empty, parseRepoArg refuses a blank one.
