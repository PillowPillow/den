...
`internal/cli/testdata/unknown-command.golden` lists the commands den understands, as `sub.Name()` plus `sub.Short`. Three things change on that one row, and missing any of them fails the test:

- the **name**: `sh` → `exec`;
- the **position**: cobra sorts the slice, so `exec` moves up between `doctor` and `init`, where `sh` sat between `rm` and `source`;
- the **description**: Step 2 rewrote `Short`, so the row now reads `Run a command in an existing sandbox, or open a shell`.

The padding does not move: it is cobra's own (`minNamePadding = 11`, widened only by a name longer than that), and the longest command name is `version` at 7. Both before and after, `pad` is 11.

The golden prints `Name()`, not `UseLine()`, so the `[-- <cmd> [args...]]` suffix Task 5 adds to `Use` never reaches this file.
...
`internal/cli/root_test.go:317` and `:322` freeze the argument refusals, which quote `cmd.CommandPath()` and `cmd.UseLine()`:
...
These strings change **again in Task 5**, and knowingly: `UseLine()` renders the `Use` field, which Task 5 rewrites to `exec <name> [-- <cmd> [args...]]`, and the second refusal is replaced outright by `execArgs`. Advertising `--` in `Use` here, before the command accepts it, would be a lie for the length of one commit — so the string is fixed twice rather than once.
...
Run: ``grep -rn '`den ' --include='*.go' . | grep -v '.claude/worktrees'``

Grep the **remedy shape**, not the phrase: den's errors name the command to run in backticks, and a remediation string still pointing at `den sh` would send a user to a command that no longer exists. `internal/sbx/ls.go` is the file to read closely — its remediation is locked by `TestVerifieEnMarcheNeSuggereQueDesCommandesAttestees`, which exists precisely to keep den from suggesting a command that is not attested. Then rewrite each `den sh` hit to `den exec`. They are comments in `internal/sbx/runner.go`, `internal/sbx/ls.go`, `internal/config/config_test.go`, `internal/cli/build.go`, `internal/cli/reference.go`, `internal/cli/nest.go`, `internal/cli/ports.go`, `internal/cli/ports_test.go`, `internal/cli/nest_test.go`, `internal/sbx/ls_test.go`, `internal/sbx/runner_test.go`, `internal/spawn/spawn.go`, `internal/spawn/spawn_test.go`. A comment naming a command that no longer exists is the kind of stale artifact this repo's CLAUDE.md keeps a section about.
...
- [ ] **Step 4b: Fix the usage strings Task 4 wrote, now that `Use` is final**

`internal/cli/root_test.go:317` quotes `cmd.UseLine()`, which now renders the `--` form:

```go
			"den exec: one argument expected, none received — usage: den exec <name> [-- <cmd> [args...]] [flags]",
```

`:322` (`exactly one argument expected, 2 received`) no longer exists as a message: `execArgs` replaces `exactlyOneArg` on this command, and two bare positionals now take the "a command must be separated by `--`" branch. Replace that case with an assertion on the new refusal:
...
Run `go test ./internal/cli/ -run TestUnknownArgument -count=1` (or whichever test in `root_test.go` owns those two strings — find it with `grep -n "one argument expected" internal/cli/root_test.go`) and match the produced text exactly.
...
- [ ] **Step 2: Check the design spec still matches what was built**

`2026-08-10-den-exec-design.md` already carries the two precisions that came out of implementation, each dated in the repo's habit of dating every correction: the tty rule governing a **given** command only, and the workdir applying to both modes. Re-read them against the code as it now stands and correct whichever drifted — a spec marked "conception validée" that no longer describes the binary is the stale artifact this repo's CLAUDE.md keeps a section about.
