...
- Test: `internal/agent/mixin_test.go`
...
Run: `go test ./internal/agent/ -run 'TestRenderMixin|TestMixinFrom' -count=1 -v`
...
Expected: PASS, including the untouched `mixin-complete.golden` — a mixin with no mounts must render byte-identically to before.

- [ ] **Step 5: Full check and commit**

```bash
task check
git add internal/agent/mixin.go internal/agent/mixin_test.go internal/agent/testdata/mixin-with-mounts.golden
git commit -m "feat(agent): emit the link phase as the first startup entry of the mixin"
```
...
### Task 8: Update spec §10 and the README
...
**Spec coverage.** Config surface → Task 1. Expansion rule → Tasks 1 and 4 (asserted in both). Global-only → Task 1's comment plus the absence of any cascade code. `ssh.mode` sugar → Task 3. `ro` → Tasks 1 and 6. Mount at create time → Task 6. Link phase first → Tasks 4 and 5. Drift for free → no task, correctly: it needs no code. `den doctor` → Task 7. Host-side gate → Task 6. Empty/non-empty table → Task 4 (`linkFunc`) and Task 9 (behaviour). `ssh.dir` message preserved → Task 6, with its own test. Tests list → distributed. §10 rewrite → Task 8. Security note → no code; it is a user-facing caveat already in the design doc, and `README.md` picks it up in Task 8 Step 3.
